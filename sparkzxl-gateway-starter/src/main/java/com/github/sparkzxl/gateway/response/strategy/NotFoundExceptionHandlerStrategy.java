...
public class NotFoundExceptionHandlerStrategy implements ExceptionHandlerStrategy {
    ...
    public ExceptionHandlerResult handleException(Throwable throwable) {
        ...
        if (message.contains(matchString)) {
            ...
            message = StrFormatter.format(ExceptionCode.OPEN_SERVICE_UNAVAILABLE.getMessage(), applicationName);
        }
        Response responseResult = Response.fail(ExceptionCode.OPEN_SERVICE_UNAVAILABLE.getCode(), message);
