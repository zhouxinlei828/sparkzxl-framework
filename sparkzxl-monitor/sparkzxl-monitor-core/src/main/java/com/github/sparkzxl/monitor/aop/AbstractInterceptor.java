...
import com.github.sparkzxl.monitor.constant.ProxyConstant;
...
public abstract class AbstractInterceptor implements MethodInterceptor {
    ...
    public String getMethodParameterTypesValue(MethodInvocation invocation) {
        ...
        return toString(parameterTypes);
    }
    ...
    public <T> T getValueByParameterAnnotation(MethodInvocation invocation, Class<?> parameterAnnotationType, Class<T> parameterType) {
        ...
        if (ArrayUtils.isEmpty(parameterAnnotations)) {
            throw new RuntimeException("Not found any annotations");
        }
        ...
        for (Annotation[] parameterAnnotation : parameterAnnotations) {
            for (Annotation annotation : parameterAnnotation) {
                if (annotation.annotationType() == parameterAnnotationType) {
                    ...
                    if (value != null) {
                        throw new RuntimeException("Only 1 annotation=" + parameterAnnotationType.getName() + " can be added in method [name=" + methodName + ", parameterTypes=" + parameterTypesValue + "]");
                    }

                    Object object = arguments[valueIndex];
                    // 方法注解的值不允许为空
                    if (object == null) {
                        throw new RuntimeException("Value for annotation=" + parameterAnnotationType.getName() + " in method [name=" + methodName + ", parameterTypes=" + parameterTypesValue + "] is null");
                    }

                    // 方法注解的类型不匹配
                    if (object.getClass() != parameterType) {
                        throw new RuntimeException("Type for annotation=" + parameterAnnotationType.getName() + " in method [name=" + methodName + ", parameterTypes=" + parameterTypesValue + "] must be " + parameterType.getName());
                    }
